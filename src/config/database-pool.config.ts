...
export const getDatabasePoolConfig = (configService: ConfigService): Partial<TypeOrmModuleOptions> => {
    const poolConfig = {
        ...
        handleDisconnects: configService.get<boolean>('DB_POOL_HANDLE_DISCONNECTS') ?? true,
    };
    ...
    return {
        ...
        extra: {
            ...
            application_name: configService.get<string>('DB_APPLICATION_NAME') || 'rampa-cash-api',
            
            // Connection string parameters
            ...(configService.get<string>('DB_EXTRA_PARAMS') && {
                ...JSON.parse(configService.get<string>('DB_EXTRA_PARAMS') || '{}'),
            }),
        },
        
        // Connection options
        connectTimeoutMS: configService.get<number>('DB_CONNECT_TIMEOUT') || 30000,
        socketTimeoutMS: configService.get<number>('DB_SOCKET_TIMEOUT') || 30000,
        
        // Logging
        logging: configService.get<boolean>('DB_LOGGING') ? ['query', 'error', 'schema', 'warn', 'info', 'log'] : false,
        logger: (configService.get<string>('DB_LOGGER') as any) || 'advanced-console',
