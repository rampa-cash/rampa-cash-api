...
export class CreateOnRampDto {
    ...
    @ApiProperty({ description: 'Fiat amount to pay', minimum: 0.01 })
    @IsNumber()
    @Min(0.01)
    fiatAmount: number;
    ...
    @ApiProperty({ description: 'Token type to receive', enum: TokenType })
    @IsEnum(TokenType)
    tokenType: TokenType;

    @ApiProperty({ description: 'On-ramp provider (e.g., stripe, sepa)' })
    @IsString()
    provider: string;
