...
export class UserCreationService {
        ...
        try {
            ...
            if (existingUser) {
                ...
                if (wasInactive) {
                    ...
                    const reactivatedUser = await this.userRepository.save(
                        existingUser,
                    );
            ...
            if (error instanceof QueryFailedError) {
                ...
                if (driverError?.code === '23505') {
                    ...
                    if (!existingUser && sessionData.phoneNumber && sessionData.phoneNumber.trim() !== '') {
                        existingUser = await this.userRepository.findOne({
                            where: { phone: sessionData.phoneNumber },
                        });
                    }
                    ...
                    if (!existingUser && driverError?.detail?.includes('phone') && driverError?.detail?.includes('()')) {
                        // Try to find user with empty phone or null phone
                        existingUser = await this.userRepository
                            .createQueryBuilder('user')
                            .where('user.phone = :emptyPhone OR user.phone IS NULL', { emptyPhone: '' })
                            .getOne();
                    }
                    ...
                    if (!existingUser && driverError?.detail) {
                        const detailMatch = driverError.detail.match(/Key \(([^)]+)\)=\(([^)]+)\)/);
                        if (detailMatch) {
                    ...
                    if (!existingUser) {
                        ...
                        if (driverError?.detail?.includes('phone') && driverError?.detail?.includes('()')) {
                            existingUser = await this.userRepository
                                .createQueryBuilder('user')
                                .where('(user.phone = :emptyPhone OR user.phone IS NULL)', { emptyPhone: '' })
                                .andWhere('user.email = :email', { email: sessionData.email || '' })
                                .getOne();
                        }
                        ...
                        if (!existingUser) {
                            ...
                            if (sessionData.email) {
                                whereConditions.push({ email: sessionData.email });
                            }
                            if (sessionData.phoneNumber && sessionData.phoneNumber.trim() !== '') {
                                whereConditions.push({ phone: sessionData.phoneNumber });
                            }
                            if (sessionData.userId) {
                                whereConditions.push({ authProviderId: sessionData.userId });
                            }
                    ...
                    if (existingUser) {
                        ...
                        this.logger.error('Unique constraint violation but user not found', {
                    ...
                    this.logger.error('QueryFailedError but not unique constraint', {
                        code: driverError?.code,
    ...
    async getUserById(userId: string, includeInactive = false): Promise<User | null> {
