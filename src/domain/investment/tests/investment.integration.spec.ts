...
describe('Investment Integration Tests', () => {
        ...
        app = await Test.createTestingModule({
            imports: [
                TypeOrmModule.forRoot({
                    type: 'sqlite',
                    database: ':memory:',
                    entities: [InvestmentOption, UserInvestment, InvestmentTransaction],
                    synchronize: true,
                }),
    ...
    describe('Investment Options Management', () => {
        it('should create and retrieve investment options', async () => {
            ...
            for (const optionData of options) {
                const option = await investmentService.createInvestmentOption(optionData);
                expect(option).toBeDefined();
                expect(option.name).toBe(optionData.name);
            }

            // Get all options
            const allOptions = await investmentController.getAllInvestmentOptions();
            expect(allOptions).toHaveLength(2);

            // Filter by type
            const etfOptions = await investmentController.getInvestmentOptionsByType(InvestmentType.TOKENIZED_ETF);
            expect(etfOptions).toHaveLength(1);
            expect(etfOptions[0].name).toBe('Tokenized Gold ETF');

            // Filter by risk level
            const highRiskOptions = await investmentController.getInvestmentOptionsByRisk(InvestmentRisk.HIGH);
            expect(highRiskOptions).toHaveLength(1);
            expect(highRiskOptions[0].name).toBe('Crypto Growth Fund');

            // Search options
            const searchResults = await investmentController.searchInvestmentOptions({ query: 'Gold' });
    ...
    describe('User Investment Flow', () => {
        it('should handle complete investment lifecycle', async () => {
            ...
            // Create investment
            const investment = await investmentService.createUserInvestment(userId, option.id, 1000);
            ...
            // Process investment
            const transaction = await investmentService.processInvestment(userId, option.id, 1000);
            expect(transaction).toBeDefined();
            expect(transaction.type).toBe(TransactionType.INVESTMENT);
            expect(transaction.amount).toBe(1000);

            // Update investment status to active
            const activeInvestment = await investmentService.updateInvestmentStatus(
                investment.id,
            ...
            const updatedInvestment = await investmentService.updateInvestmentValue(
                investment.id,
                1100,
            );
            ...
            // Pause investment
            const pausedInvestment = await investmentService.pauseInvestment(investment.id);
            expect(pausedInvestment.status).toBe(InvestmentStatus.PAUSED);

            // Resume investment
            const resumedInvestment = await investmentService.resumeInvestment(investment.id);
            expect(resumedInvestment.status).toBe(InvestmentStatus.ACTIVE);

            // Process withdrawal
            const withdrawal = await investmentService.processWithdrawal(userId, investment.id, 200);
            expect(withdrawal).toBeDefined();
            expect(withdrawal.type).toBe(TransactionType.WITHDRAWAL);
            expect(withdrawal.amount).toBe(200);

            // Cancel investment
            const cancelledInvestment = await investmentService.cancelInvestment(investment.id);
    ...
    describe('Investment Statistics and Analytics', () => {
        it('should provide investment statistics', async () => {
            ...
            for (let i = 0; i < createdOptions.length; i++) {
                ...
                await investmentService.updateInvestmentStatus(investment.id, InvestmentStatus.ACTIVE);

                // Update values with different returns
                const newValue = 1000 * (i + 1) * (1.1 + i * 0.05); // 10% and 15% returns
                await investmentService.updateInvestmentValue(investment.id, newValue);
            }

            // Get user stats
            const stats = await investmentService.getUserInvestmentStats(userId);
            ...
            const performance = await investmentService.getUserInvestmentPerformance(userId);
            expect(performance).toHaveLength(2);
            expect(performance[0].amount).toBe(1000);
            expect(performance[1].amount).toBe(2000);

            // Get top performing investments
            const topPerforming = await investmentService.getTopPerformingInvestments(5);
    ...
    describe('Investment Option Performance', () => {
        it('should track investment option performance', async () => {
            ...
            const performance = await investmentService.getInvestmentOptionPerformance(option.id);
