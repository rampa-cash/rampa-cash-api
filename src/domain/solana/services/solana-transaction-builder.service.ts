...
export class SolanaTransactionBuilderService {
     ...
     */
    async buildTransaction(request: TransactionBuildRequest): Promise<TransactionBuildResult> {
        try {
            this.logger.debug(`Building transaction from ${request.fromAddress} to ${request.toAddress} for ${request.amount} ${request.token}`);

            // Validate addresses
            if (!this.isValidAddress(request.fromAddress)) {
                throw new BadRequestException(`Invalid sender address: ${request.fromAddress}`);
            }

            if (!this.isValidAddress(request.toAddress)) {
                throw new BadRequestException(`Invalid recipient address: ${request.toAddress}`);
            }
            ...
            if (request.token === TokenType.SOL) {
                ...
                const transferInstruction = await this.splTokenService.createTransferInstruction(
                    fromPublicKey,
                    toPublicKey,
                    request.amount,
                    request.token,
            ...
            const { blockhash } = await this.connectionService.getConnection().getLatestBlockhash();
            ...
            this.logger.log(`Transaction built successfully. Estimated fee: ${estimatedFee} lamports`);
            ...
            this.logger.error(`Failed to build transaction: ${error.message}`, error.stack);
     ...
     */
    async estimateTransactionFee(request: TransactionBuildRequest): Promise<number> {
        try {
            const result = await this.buildTransaction(request);
            return result.estimatedFee;
        } catch (error) {
            this.logger.error(`Failed to estimate transaction fee: ${error.message}`, error.stack);
            throw new BadRequestException(`Failed to estimate transaction fee: ${error.message}`);
        }
    }
     ...
     */
    async getTransactionSize(request: TransactionBuildRequest): Promise<number> {
        try {
            const result = await this.buildTransaction(request);
            return result.transaction.serialize({ requireAllSignatures: false }).length;
        } catch (error) {
            this.logger.error(`Failed to get transaction size: ${error.message}`, error.stack);
            throw new BadRequestException(`Failed to get transaction size: ${error.message}`);
        }
    }
     ...
     */
    async validateTransactionSize(request: TransactionBuildRequest): Promise<boolean> {
        try {
            const size = await this.getTransactionSize(request);
            const maxSize = 1232; // Solana transaction size limit
            return size <= maxSize;
        } catch (error) {
            this.logger.error(`Failed to validate transaction size: ${error.message}`, error.stack);
            return false;
        }
    }
}
