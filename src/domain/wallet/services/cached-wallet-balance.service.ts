...
export class CachedWalletBalanceService {
    ...
    async getBalance(walletId: string, tokenType: TokenType): Promise<number> {
        const cacheKey = this.cacheService.getWalletBalanceKey(walletId, tokenType);
        
        // Try to get from cache first
        const cachedBalance = await this.cacheService.get<number>(cacheKey);
        if (cachedBalance !== null) {
            this.logger.debug(`Cache hit for wallet ${walletId} balance ${tokenType}`);
            return cachedBalance;
        }

        // Cache miss - get from service
        this.logger.debug(`Cache miss for wallet ${walletId} balance ${tokenType}`);
        const balance = await this.walletBalanceService.getBalance(walletId, tokenType);
    ...
    async getAllBalances(walletId: string): Promise<WalletBalance[]> {
        ...
        // Try to get from cache first
        const cachedBalances = await this.cacheService.get<WalletBalance[]>(cacheKey);
    ...
    async updateBalance(
        ...
        this.logger.debug(`Updated balance for wallet ${walletId} ${tokenType} and invalidated cache`);
    ...
    async addBalance(
        ...
        this.logger.debug(`Added balance for wallet ${walletId} ${tokenType} and invalidated cache`);
    ...
    async subtractBalance(
        ...
        this.logger.debug(`Subtracted balance for wallet ${walletId} ${tokenType} and invalidated cache`);
    ...
    async initializeWalletBalances(walletId: string): Promise<void> {
        ...
        this.logger.debug(`Initialized balances for wallet ${walletId} and invalidated cache`);
    }
    ...
    async getBalanceFromDatabase(walletId: string, tokenType: TokenType): Promise<number> {
        return await this.walletBalanceService.getBalanceFromDatabase(walletId, tokenType);
    }
    ...
    async getAllBalancesFromDatabase(walletId: string): Promise<WalletBalance[]> {
        return await this.walletBalanceService.getAllBalancesFromDatabase(walletId);
    }

    /**
     * Invalidate cache entries for a specific wallet balance
     */
    private async invalidateWalletBalanceCache(walletId: string, tokenType: TokenType): Promise<void> {
        const balanceKey = this.cacheService.getWalletBalanceKey(walletId, tokenType);
        const allBalancesKey = this.cacheService.getAllWalletBalancesKey(walletId);
