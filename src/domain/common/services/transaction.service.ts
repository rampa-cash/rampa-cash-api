...
export class DatabaseTransactionService {
    ...
    async executeInTransaction<T>(
        ...
        try {
            ...
            this.logger.error('Database transaction rolled back due to error:', error);
            throw error;
        } finally {
    ...
    async executeInReadOnlyTransaction<T>(
        ...
        try {
            ...
            await queryRunner.commitTransaction();
            this.logger.debug('Read-only database transaction completed successfully');
    ...
    async executeWithRetry<T>(
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                this.logger.debug(`Executing transaction operation (attempt ${attempt}/${maxRetries})`);
    ...
    async executeInParallel<T>(
        ...
        return await this.executeInTransaction(async (queryRunner) => {
            this.logger.debug(`Executing ${operations.length} operations in parallel`);
